...
//Export module
	...
	io.sockets.on('connection', function (socket) {
		...
		socket.on('adduser', function(username, roomType){
			...
			console.log('language: ' + i18n.__('Test'));
			...
			if (username !== undefined && username !== null && username !== "" &&
				...
				ot.createSession(config.app.hostname, {}, function(e, session) {
					socket.userData = {
						sessionId: session,
		...
		socket.on('user image', function (message) {
			...
			if (socket.partner){
				...
				if (partnerSocket){
					...
					logger.error('chatConfig', ' - ', 'userImage', ' - ', 'no partnerSocket exist');
				}
			}
			else{
				logger.error('chatConfig', ' - ', 'userImage', ' - ', 'no socket.partner exist');
			}
		});

		/*userSound:
		 Send voice note to the chat
		 * message: sound that will be sent to the chat
		 ***/
		socket.on('user sound', function (message) {
			...
			if (socket.partner){
				...
				if (partnerSocket){
					...
					logger.error('chatConfig', ' - ', 'userSound', ' - ', 'no partnerSocket exist');
				}
			}
			else{
				logger.error('chatConfig', ' - ', 'userSound', ' - ', 'no socket.partner exist');
			}
		});
		...
		socket.on('sendchat', function (message) {
			...
			if (socket.partner){
				...
				if (partnerSocket){
					...
					logger.error('chatConfig', ' - ', 'sendchat', ' - ', 'no partnerSocket exist');
				}
			}
			else{
				logger.error('chatConfig', ' - ', 'sendchat', ' - ', 'no socket.partner exist');
			}
		});
		...
		socket.on('userWriting', function(){
			if (socket.partner){
				...
				if (partnerSocket){
					...
					logger.error('chatConfig', ' - ', 'userWriting', ' - ', 'no partnerSocket exist');
				}
			}
			else{
				logger.error('chatConfig', ' - ', 'userWriting', ' - ', 'no socket.partner exist');
			}
		});
		...
		socket.on('userNotWriting', function(){
			if (socket.partner){
				...
				if (partnerSocket){
					...
					logger.error('chatConfig', ' - ', 'userNotWriting', ' - ', 'no partnerSocket exist');
				}
			}
			else{
				logger.error('chatConfig', ' - ', 'userNotWriting', ' - ', 'no socket.partner exist');
			}
		});
			...
			if (socket.partner && socket.partner.socketId) {
				...
				if (partnerSocket){
					if (type && type === 'text'){
						...
						//Desiconnect me from my partner published stream
						...
						soloUsersText.push({socketId: socket.id});
					...
					logger.error('chatConfig', ' - ', 'disconnectPartners', ' - ', 'no partnerSocket exist');
				}
			}
			else{
				logger.error('chatConfig', ' - ', 'disconnectPartners', ' - ', 'no socket.partner exist');
			}
		});
			...
			if (filteredArray.length > 0){
				...
				if (socket.partner !== tmpUser && tmpUser !== me) {
					...
					if (partnerSocket) {
						...
						logger.error('chatConfig', ' - ', 'nextVideo', ' - ', 'no partnerSocket exist');
					}
				}
			}
		...
		socket.on('nextTextVideo', function(){
			if (socket.partner) {
				...
				logger.error('chatConfig', ' - ', 'nextTextVideo', ' - ', 'no socket.partner exist');
			}
		});
		...
		socket.on('newVideoChat2', function() {
			...
			if (socket.partner){
				...
				if (partnerSocket){
					...
					logger.error('chatConfig', ' - ', 'newVideoChat2', ' - ', 'no partnerSocket exist');
				}
			}
			else{
				logger.error('chatConfig', ' - ', 'newVideoChat2', ' - ', 'no socket.partner exist');
			}
		});
		...
		socket.on('succesNewVideoChat', function() {
			...
			if (socket.partner){
				...
				if (partnerSocket){
					...
					logger.error('chatConfig', ' - ', 'succesNewVideoChat', ' - ', 'no partnerSocket exist');
				}
			}
			else{
				logger.error('chatConfig', ' - ', 'succesNewVideoChat', ' - ', 'no socket.partner exist');
			}
		});
		...
		socket.on('failNewVideoChat', function(){
			...
			if (socket.partner){
				...
				if (partnerSocket){
					...
					logger.error('chatConfig', ' - ', 'failNewVideoChat', ' - ', 'no partnerSocket exist');
				}
			}
			else{
				logger.error('chatConfig', ' - ', 'failNewVideoChat', ' - ', 'no socket.partner exist');
			}
		});
		...
		socket.on('cancelVideoChat', function(){
			...
			if (socket.partner){
				...
				if (partnerSocket){
					...
					logger.error('chatConfig', ' - ', 'cancelVideoChat', ' - ', 'no partnerSocket exist');
				}
			}
			else{
				logger.error('chatConfig', ' - ', 'cancelVideoChat', ' - ', 'no socket.partner exist');
			}
		});
		...
		socket.on('cancelPusblish', function(){
			...
			if (socket.partner){
				...
				if (partnerSocket){
					...
					logger.error('chatConfig', ' - ', 'cancelPusblish', ' - ', 'no partnerSocket exist');
				}
			}
			else{
				logger.error('chatConfig', ' - ', 'cancelPusblish', ' - ', 'no socket.partner exist');
			}
		});
