  ...
  io.sockets.on('connection', function (socket) {
    ...
    socket.on('adduser', function(username, roomType){
      ...
      if (username !== undefined && username !== null && username !== "" &&
        ...
        if (roomType !== 'text'){
          ...
          socket.emit('initialVideo', socket.userData, 'video');
        }
        else{
    ...
    socket.on('nextVideo', function(){
      ...
      if (partner) {
        ...
        socket.emit('updatechat', 'SERVER', '<span class="muted">you have connected to this room</span>', 'connect', 'video');

        //Update partner window with connected message
        partnerSocket.emit('updatechat', 'SERVER', '<span class="muted">'+ "Anonym" + ' has connected to this room</span>', 'connect', 'video');
    ...
    socket.on('newVideoChat', function(roomType, typeUser) {
      if (roomType && roomType === 'text'){
        if (typeUser && typeUser === 'Anonym'){
          socket.emit('initialVideo', socket.userData, 'text', 'Anonym');
        }
        else{
          socket.emit('initialVideo', socket.userData, 'text');
        }
      }
      else{
        socket.emit('initialVideo', socket.userData, 'video');
