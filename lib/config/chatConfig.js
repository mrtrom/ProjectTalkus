...
//Export module
	...
	io.sockets.on('connection', function (socket) {
		 ...
		 ***/
        ss(socket).on('user sound', function(stream, data) {
            stream.on('data', function(chunk) {
                console.log(chunk);
            });
            stream.on('end', function() {
                socket.emit('updatechat', socket.userData.username, data.name, 'message', 'me' , false , true);
                var partnerSocket = GLOBAL.globalChatUsers[socket.partner.socketId];
                partnerSocket.emit('updatechat', socket.userData.username, data.name, 'message' , 'partner' , false , true);
            });
        });
		/*socket.on('user sound', function (message) {
			...
			if (socket.partner){
				...
				if (partnerSocket){
					...
					logger.info('chatConfig', ' - ', 'userSound', ' - ', 'no partnerSocket exist');
				}
			}
			else{
				logger.info('chatConfig', ' - ', 'userSound', ' - ', 'no socket.partner exist');
			}
		});*/
